...
5. 代码生成器支持 生成父类为TreeEntity类的结构
6. 代码生成器支持 解析数据库注释中的@InjectionField类型
7. 数据注入工具 重命名为： zuihou-injection-starter
8. 修复dozer-starter的配置读取无法使用通配符的bug 考虑 废弃 dozer ，采用hutool 的BeanUtil 作为JavaBean 的转换工具。
