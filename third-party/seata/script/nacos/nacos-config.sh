...
function addConfig() {
  curl -X POST -H ${1} "http://$2/nacos/v1/cs/configs?dataId=$3&group=SEATA_GROUP&content=$4&tenant=$5" >${tempLog} 2>/dev/null
...
for line in $(cat $(dirname "$PWD")/config.txt); do
  (( count++ ))
    key=${line%%=*}
  value=${line#*=}
    addConfig ${contentType} ${nacosAddr} ${key} ${value} ${tenant}
done
